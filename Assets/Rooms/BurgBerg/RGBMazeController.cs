...
public class RGBMazeController : MonoBehaviour {
    ...
    [SerializeField] RGBCell[,] cells;

    public static float defaultInc = 1 / 16f;
    [Range(1 / 64.0f, 1f)]
    public float waterIncrement = defaultInc;
    [Range(1 / 64.0f, 1f)]
    public float lavaIncrement = defaultInc;
    [Range(1 / 64.0f, 1f)]
    public float grassIncrement = defaultInc;
    ...
    private void InitializeMaze() {
        ...
        Vector3 center = new Vector3(height / 2, height, width *.45f);
        ...
        int removePer = UnityEngine.Random.Range(0, 40);

        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                int percent = UnityEngine.Random.Range(0, 100);
                if (board[i, j] == WALL && percent < removePer) {
                    board[i, j] = PATH;
                }
            }
        }
        // Make sure there isn't a PATH surrounded by WALLS
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                if (board[i, j] == PATH) {
                    List<DestAndPos> spots = GetValidNeighbors(i, j);
                    int walls = 0;
                    ...
                    if (walls == spots.Count) {
                        DestAndPos toRemove = spots[spots.Count - 1];
                        // Arbitrarily remove the last wall on the list.
                        board[(int)toRemove.dest.x, (int)toRemove.dest.z] = PATH;
                    }
                }
            }
        }
        ...
        //Array of Cells
        cells = new RGBCell[height, width];

        //Generate Blocks
        for (int x = -1; x <= height; ++x) {
            for (int z = -1; z <= width; ++z) {
                //Place Block
                RGBBlock block = PlaceNewBlock(x, z);
                if (x >= 0 && z >= 0 && z < width && x < height) {
                    cells[x, z] = new RGBCell(block);
                    if (block != null)
        ...
        int numStarts = UnityEngine.Random.Range(1, 3*sizeMultiplier);
                ...
                do {
                    ...
                    if (board[x, z] == PATH) {
                        ...
                        if (cells[x, z].Block1 != null && cells[x, z].Block1.gameObject != null) {
                            AllGameObjects.Remove(cells[x, z].Block1.gameObject);
                            Destroy(cells[x, z].Block1.gameObject);
                        }
                        cells[x, z].Block1 = new RGBBlock(startObj);
    ...
    private RGBBlock PlaceNewBlock(int x, int z) {
        GameObject go;
        if (x < 0 || z < 0 || z >= width || x >= height) {
    ...
    public void GrowBlock(RGBBlock block) {
    ...
    public void ShrinkSecondBlock(RGBCell cell) {
        RGBBlock block = cell.Block2;
    ...
    private void ShrinkAndGrowBlock(RGBCell cell) {
    ...
    private bool Propigate(RGBBlock block, int x, int z) {
        if (block.gameObject.CompareTag("Dirt") || block.gameObject.CompareTag("Path")) {
            return false;
        }
    ...
    private bool PropigateGrass(List<DestAndPos> spots) {
        ...
        foreach (DestAndPos p in spots) {
            RGBCell cell = cells[(int)p.dest.x, (int)p.dest.z];
            if (cell.Block1 == null) {
                addGrass(p);
                surrounded++;
    ...
    private void AddBlockToCell(GameObject gameObject, Type block, float inc, string name, DestAndPos p) {
        ...
        RGBBlock b = (RGBBlock)Activator.CreateInstance(block, paramList);
